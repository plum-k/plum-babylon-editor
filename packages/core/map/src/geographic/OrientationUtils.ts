...
type string = ProjectionDefinition | string;
...
export function quaternionFromOmegaPhiKappa(
    ...
    // <=> target.multiply(new Quaternion(1, 0, 0, 0));
    return target;
}
...
export function quaternionFromEnuToGeocent(coordinates?: Coordinates, target = new Quaternion()) {
    ...
    return (coordinates: Coordinates, target = new Quaternion()) => {
        ...
        // matrix.lookAt(up, new Vector3(), new Vector3(0, 0, 1));
...
export function quaternionFromEnuToCRS(proj: string): QuaternionFunction;
...
export function quaternionFromCRSToEnu(proj: string): QuaternionFunction;
