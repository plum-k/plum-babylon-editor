...
import {Vector2, Vector3,BoundingBox,Vector4 } from '@babylonjs/core';
import type {string} from './Crs';

const _dim = new Vector2();
const _dim2 = new Vector2();
const _box = new BoundingBox(new Vector3(),new Vector3());
...
export class Extent {
     ...
     * @returns A {@link Vector4} where the `x` property encodes the
     ...
     */
    static fromBox3(crs: string, box: Box3) {
        if (CRS.isGeocentric(crs)) {
